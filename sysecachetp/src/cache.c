...
int add_cache(char* name, int len, long long file_len, char** buf)
{
    ...
    cc->cache_buf = buf;
    cc->next = cache_head; 
    if(cache_rear == NULL) cache_rear = cc;
    cache_head = cc; 
    cache_len++;
    if(!select_lru) cc->times = 1; 
    cache_content* hash_p = hash_t[name[0] % 500];
    if(hash_p == NULL)
    {
        hash_t[name[0] % 10] = cc;
        cc->hash_before = NULL;
...
cache_content* search_cache(char* name)
{
    ...
    if(strcmp(hash_p->this_name, name) == 0)
    { 
        pthread_mutex_unlock(&(mutex));
        return hash_p;
    }
...
int delete_cache()
{
    ...
    if(select_lru)
    {
        ...
        while(cc->next != cache_rear && cc->next != NULL) cc = cc->next;
